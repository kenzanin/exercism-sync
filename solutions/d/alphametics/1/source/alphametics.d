...
{
    // parse equation
    auto parts = puzzle.split("==");
    auto left = parts[0].strip;
    auto right = parts[1].strip;

    string[] addends;
    foreach (part; left.split('+'))
        addends ~= part.strip;
    string result = right;
    ...
    if (letters.length > 10)
        assertThrown("too many letters");

    // leading letters can't be zero
    string leading;
    foreach (w; addends)
        if (w.length > 1 && leading.indexOf(w[0]) < 0)
            leading ~= w[0];
    if (result.length > 1 && leading.indexOf(result[0]) < 0)
        leading ~= result[0];

    size_t n = letters.length;
    int[] assign = new int[n];
    bool[10] used = [false, false, false, false, false, false, false, false, false, false];
    bool found = false;
    string finalLeft;
    string finalRes;
    ...
    void backtrack(int idx)
    {
        ...
        if (idx == n)
        {
            ...
            foreach (w; addends)
            {
                ...
                foreach (c; w)
                {
                    auto i = letters.indexOf(c);
                    s ~= to!string(assign[i]);
                }
            ...
            {
                auto i = letters.indexOf(c);
                sres ~= to!string(assign[i]);
            }
            if (sres.length > 1 && sres[0] == '0') return;
            if (sum == BigInt(sres))
            {
                string leftStr;
                foreach (w; addends)
        ...
        foreach (d; 0 .. 10)
        {
            if (used[d]) continue;
            auto ch = letters[idx];
            if (d == 0 && leading.indexOf(ch) >= 0) continue;
            used[d] = true;
            assign[idx] = d;
            backtrack(idx + 1);
            used[d] = false;
    ...
    backtrack(0);
    if (!found) assertThrown("no solution");
    return finalLeft ~ " == " ~ finalRes;
}
