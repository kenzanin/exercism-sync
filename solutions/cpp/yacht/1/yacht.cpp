...
int score(const std::vector<int> &in1, std::string_view const &in2) {
  ...
  auto full_house = [&in1]() -> int {
    ...
    if (var01.size() != 2) {
      return 0;
    } else if (var01.begin()->second > 3 || (var01.begin()++)->second > 3) {
      return 0;
    }
  ...
  auto little_straight = [&in1]() -> int {
    auto y = std::vector<int>{in1};
    std::sort(y.begin(), y.end());
    return y == std::vector<int>{1, 2, 3, 4, 5} ? 30 : 0;
  };
  auto big_straight = [&in1]() -> int {
    auto y = std::vector<int>{in1};
    std::sort(y.begin(), y.end());
    return y == std::vector<int>{2, 3, 4, 5, 6} ? 30 : 0;
  };
  auto choice = [&in1]() -> int {
    return std::accumulate(in1.begin(), in1.end(), 0);
  };
